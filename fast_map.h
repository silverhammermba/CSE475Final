...
{
public:
	using hash_t = std::function<size_t(K)>;
	using pair_t = std::pair<K, V>;
	using element_t = std::unique_ptr<pair_t>;
	using element_list_t = std::vector<element_t>;
	using hashed_element_list_t = std::vector<element_list_t>;
	using ptr_t = std::unique_ptr<FastLookupMap<K, V>>;
	using table_t = std::vector<ptr_t>;
